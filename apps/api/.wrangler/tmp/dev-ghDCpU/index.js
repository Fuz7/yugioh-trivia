...
var authController = {
  ...
  async loginUser(c) {
    try {
      const db = c.get("db");
      const body = await c.req.json();
      console.log(await c.req);
      const user = await authService.login(db, body);
      const accessToken = await sign(user, c.env.JWT_SECRET, "HS256");
...
// .wrangler/tmp/bundle-exrqUd/middleware-insertion-facade.js
...
// .wrangler/tmp/bundle-exrqUd/middleware-loader.entry.ts
