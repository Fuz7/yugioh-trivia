...
var usersController = {
  getUser(c) {
    const { exp, ...user } = c.get("jwtPayload");
    return c.json(user);
  }
};
...
usersRouter.get("/me", users_controller_default.getUser);
...
async function hashPassword(password) {
  ...
  const toHex = /* @__PURE__ */ __name((buf) => [...new Uint8Array(buf)].map((b) => b.toString(16).padStart(2, "0")).join(""), "toHex");
...
var authController = {
  ...
  async loginUser(c) {
    try {
      ...
      const accessToken = await sign(
        { ...user, exp: Math.floor(Date.now() / 1e3) + 60 * 15 },
  ...
  async refreshToken(c) {
    try {
      ...
      const accessToken = await sign(
        { ...payload, exp: Math.floor(Date.now() / 1e3) + 60 * 15 },
        // 15 min
        c.env.JWT_ACCESS_SECRET,
        "HS256"
      );
...
// src/api.ts
var api = new Hono2();
api.get("/", (c) => c.text("Hello Hono API \u{1F680}"));
...
// src/db/client.ts
var createDB = /* @__PURE__ */ __name((D1) => drizzle(D1), "createDB");
...
// .wrangler/tmp/bundle-smgrLp/middleware-insertion-facade.js
...
// .wrangler/tmp/bundle-smgrLp/middleware-loader.entry.ts
